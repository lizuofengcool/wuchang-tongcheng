...
- v2.18.0 - core/router 路由封装单元测试补齐（此前 0 覆盖）
  - backend/internal/core/router/router_test.go 37 用例，补齐此前 0 覆盖的 router.go（Gin 路由封装 + plugin.RouterGroup/plugin.Context 适配器 + multipart.FileHeader/plugin.ResponseWriter 适配器 + convertHandlers/WrapGin 转换），与既有 middleware/plugin 包测试同风格（gin.TestMode + httptest + testify，无 DB/Redis/Docker 依赖）。覆盖 NewRouter/Engine（非 nil + 多次调用同指针）+ Router 级 GET/POST/PUT/DELETE/PATCH/Any 方法注册与命中（Any 五方法全命中）+ Use 全局中间件头注入 + Group 根路由组（plugin.HandlerFunc）与 GroupFunc 原生 gin.RouterGroup 双通道注册 + ServeHTTP 实现 http.Handler 接口 + Run 非法端口立即返回错误 + 404 未注册路由 + RouterGroup 全方法（GET/POST/PUT/DELETE/PATCH 经 plugin.HandlerFunc）+ 嵌套子组 Group + 组中间件透传（Set/Get 串行）+ convertHandlers 闭包变量隔离（多 handler 各设独立 key 不退化为最后一个 + 多 handler 路由串行执行）+ convertHandlers 空入参 + WrapGin 包装 gin 中间件经 RouterGroup 注册执行 + WrapGin 非 *Context 入参早返回（fakeContext 类型断言失败不调用 mw 不 panic）+ Context 适配器全方法（JSON/Param/Query/DefaultQuery/PostForm/Bind 成功+非法 JSON 错误/Set+Get 命中+未命中/GetHeader 命中+缺失/Status/Writer Write+WriteHeader+Header map 赋值/Request 按设计返回 nil/GinContext 返回底层 gin.Context）+ NewContext 同指针 + FormFile multipart 上传（fileHeader Filename/Size/Open 三方法 + 读取内容）+ FormFile 缺 file 字段错误 + fileHeader.Open 全量读取 + 接口编译期断言（RouterGroup/Context/FileHeader/ResponseWriter/http.Handler）
  - mock 策略：newTestRouterWith 构造测试 Router 并注册单条 plugin.HandlerFunc 路由，返回 **Context（指向捕获指针的地址）供调用方在 ServeHTTP 后解引用取得 *Context（直接返回 *Context 值拷贝会丢失闭包内赋值）；需要 post-serve 读取的 Set/Get/GetHeader 用例通过解引用 *capturedPtr 取得 *Context（gin 在 ServeHTTP 结束后不调 reset，Keys/Header 字段稳定可读）；Request/GinContext 用例改为 handler 内捕获结果经 JSON 响应体回传避免 post-serve 读取；fakeContext 实现 plugin.Context 接口供 WrapGin 非 *Context 分支触发；newMultipartBody 用 mime/multipart.Writer 构造含 "file" 字段的 multipart/form-data 请求体；-count=1 与 -race 运行通过，无全局状态污染
- v2.19.0 - pkg/seed 种子数据纯数据一致性单元测试补齐（此前 0 覆盖，补齐最后一个无测试的 internal/pkg 包）
  - backend/internal/pkg/seed/seed_test.go 16 用例，补齐此前 0 覆盖的 seed.go 包级种子数据定义内部一致性校验，与 error_code_test.go（v2.15.0）同风格：plain testing，无 testify/DB/Redis/Docker 依赖，仅校验包级数据定义防止新增条目引入重复码/无效引用/格式错误。覆盖 permissionDefs（无重复权限码 + 字段合法性：Code/Name 非空 + resource:action 格式 + Type ∈ {1菜单,2按钮,3接口}）+ businessRoleDefs（无重复角色码 + 字段合法性：Name/Code/Description 非空 + Sort 正数且唯一 + 降序排列保证菜单顺序稳定）+ rolePermMap（所有 key 命中 businessRoleDefs 且不含 super_admin 键 + 每个角色权限列表无重复权限码 + 权限码格式校验 + 每个业务角色都有权限矩阵条目防遗漏）+ moduleDefs（middleware+business 跨集合无重复模块名 + Category 与所属切片一致 + Name/DisplayName/Icon 非空 + Dependencies 合法 JSON 数组 + 依赖名引用已知模块防拼写错误）+ p0CronJobDefs（(module_name,job_name) 唯一键无重复 + CronExpr 6 字段秒分时日月周 + Params 合法 JSON + TimeoutSeconds 正数 + MaxRetry 非负）+ 跨结构聚合（permissionDefs ≥30 + businessRoleDefs=9 + rolePermMap 与 businessRoleDefs 数量一致 + middlewareModuleDefs=12 + businessModuleDefs=15 防御性规模断言）
  - 关键说明：rolePermMap 引用的部分权限码（ershou:read/order:create/rider:deliver/vip:privilege 等）由各业务模块插件路由或独立 SeedXxx 函数注册，不在 permissionDefs 中定义，故测试仅校验码格式与角色内去重，不强制要求引用码全部命中 permissionDefs，避免误报跨模块权限码。与 error_code_test.go 同风格：isPermissionCodeFormat 辅助函数校验 resource:action 单冒号格式；-count=1 与 -race 运行通过，无全局状态污染

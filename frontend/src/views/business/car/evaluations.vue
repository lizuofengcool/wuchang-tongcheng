<template>
  <div class="app-container">
    <div class="page-card">
      <el-form :inline="true" :model="filters" class="filter-form" @submit.prevent>
        <el-form-item label="关键词">
          <el-input v-model="filters.keyword" placeholder="车源/评估单号" clearable style="width: 200px" :prefix-icon="Search" @keyup.enter="onSearch" />
        </el-form-item>
      ...
      <el-table v-loading="loading" :data="list" border stripe>
        ...
        <el-table-column prop="evaluation_no" label="评估单号" width="180" />
        <el-table-column label="车源" min-width="180" show-overflow-tooltip>
          <template #default="{ row }">{{ row.car_title || `车源#${row.car_id}` }}</template>
        </el-table-column>
        <el-table-column label="评估价" width="120">
          <template #default="{ row }">¥{{ Number(row.eval_price || 0).toFixed(2) }}万</template>
        </el-table-column>
        <el-table-column label="市场价" width="120">
          <template #default="{ row }">¥{{ Number(row.market_price || 0).toFixed(2) }}万</template>
        </el-table-column>
        ...
        <el-table-column label="评估时间" width="160">
          <template #default="{ row }">{{ formatTime(row.evaluated_at || row.created_at) }}</template>
        </el-table-column>
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" link size="small" @click="openDetail(row)">详情</el-button>
            <el-button v-if="row.status === 0" type="success" link size="small" @click="handleStatus(row, 1)">发布</el-button>
    ...
    <el-dialog v-model="detailVisible" title="评估详情" width="700px" destroy-on-close>
      <el-descriptions v-if="detail" :column="2" border>
        <el-descriptions-item label="ID">{{ detail.id }}</el-descriptions-item>
        <el-descriptions-item label="评估单号">{{ detail.evaluation_no }}</el-descriptions-item>
        <el-descriptions-item label="车源">{{ detail.car_title || `车源#${detail.car_id}` }}</el-descriptions-item>
        <el-descriptions-item label="评估师">{{ detail.appraiser || '-' }}</el-descriptions-item>
        <el-descriptions-item label="评估价">¥{{ Number(detail.eval_price || 0).toFixed(2) }}万</el-descriptions-item>
        <el-descriptions-item label="市场价">¥{{ Number(detail.market_price || 0).toFixed(2) }}万</el-descriptions-item>
        <el-descriptions-item label="车况评分">{{ detail.condition_score || '-' }}</el-descriptions-item>
        <el-descriptions-item label="状态">{{ statusText(detail.status) }}</el-descriptions-item>
        <el-descriptions-item label="评估时间">{{ formatTime(detail.evaluated_at) }}</el-descriptions-item>
...
import { adminListEvaluations, adminGetEvaluation, adminUpdateEvaluationStatus } from '@/api/car'
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await adminListEvaluations(buildParams())
    const d = res.data || {}
    list.value = d.list || []
...
const openDetail = async (row) => {
  try {
    const res = await adminGetEvaluation(row.id)
...
const handleStatus = async (row, status) => {
  try {
    await adminUpdateEvaluationStatus(row.id, { status })

<template>
  <div class="app-container">
    <div class="page-card">
      <el-form :inline="true" :model="filters" class="filter-form" @submit.prevent>
        <el-form-item label="关键词">
          <el-input v-model="filters.keyword" placeholder="分类名" clearable style="width: 200px" :prefix-icon="Search" @keyup.enter="onSearch" />
        </el-form-item>
    ...
    <el-dialog v-model="formVisible" :title="formMode === 'create' ? '新建分类' : '编辑分类'" width="500px" destroy-on-close>
...
import { listCategories, createCategory, updateCategory, deleteCategory, updateCategoryStatus, getCategory } from '@/api/house'
...
const form = reactive({ id: null, name: '', parent_id: null, sort: 0, icon: '', description: '' })
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await listCategories(buildParams())
    const d = res.data || {}
    list.value = d.list || []
    total.value = d.total || 0
    parentOptions.value = list.value.filter(c => !c.parent_id)
...
const resetForm = () => Object.assign(form, { id: null, name: '', parent_id: null, sort: 0, icon: '', description: '' })
...
const handleStatus = async (row, status) => {
  try {
    await ElMessageBox.confirm(`确定${status === 1 ? '启用' : '禁用'}该分类吗？`, '提示', { type: 'warning' })
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm('确定删除该分类吗？', '危险操作', { type: 'error' })

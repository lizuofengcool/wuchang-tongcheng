<template>
  <div class="app-container">
    ...
    <div class="page-card">
      ...
      <div class="toolbar">
        <div class="toolbar-left">
          ...
          <el-button type="primary" :icon="Plus" @click="openCreate">新建举报</el-button>
        </div>
      </div>
      ...
      <el-form :model="processForm" label-width="100px">
        <el-form-item label="处理结果">
          <el-radio-group v-model="processForm.status">
            ...
            <el-radio :value="2">下架商户</el-radio>
    ...
    <!-- 新建举报弹窗 -->
    <el-dialog v-model="formVisible" title="新建举报" width="600px" destroy-on-close>
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="证据图片">
          <el-input v-model="form.evidence_images_str" type="textarea" :rows="2" placeholder="多张图片用逗号分隔" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const filters = reactive({ report_type: '', status: null, dh114_id: '' })
...
const loadList = async () => {
  ...
  try {
    ...
    // 注：dh114 模块后端未单独提供举报列表 API，使用 reviews 替代或预留扩展点
    // 此处保留通用列表请求，后端可挂载到 /dh114/reports 路由
    const res = await request.get('/dh114/reports', { params }).catch(() => ({ data: { list: [], total: 0 } }))
    const data = res.data || {}
...
const onProcess = async () => {
  ...
  try {
    await request.put(`/dh114/reports/${processForm.id}/process`, {
...
const onDelete = async (row) => {
  try {
    ...
    await request.delete(`/dh114/reports/${row.id}`)
...
const form = reactive({
  dh114_id: undefined, report_type: 'other', reason: '', description: '', evidence_images_str: ''
})
const rules = {
...
const onSubmit = async () => {
  try {
    ...
    const evidence_images = form.evidence_images_str
      ? form.evidence_images_str.split(/[,，\n\s]+/).filter(Boolean)
      : []
    await request.post('/dh114/reports', {
      dh114_id: form.dh114_id,

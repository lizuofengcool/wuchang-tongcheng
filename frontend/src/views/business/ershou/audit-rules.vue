...
import {
  listErshouAuditRules, createErshouAuditRule, updateErshouAuditRule, deleteErshouAuditRule
} from '@/api/ershou'
...
const loadList = async () => {
  loading.value = true
  try {
    const res = await listErshouAuditRules({
      page: page.value,
...
const onSubmit = async () => {
  try {
    ...
    if (isEdit.value) {
      await updateErshouAuditRule(form.id, payload)
    } else {
      await createErshouAuditRule(payload)
    }
...
const onDelete = async (row) => {
  try {
    ...
    await deleteErshouAuditRule(row.id)
...
const onToggle = async (row, val) => {
  try {
    await updateErshouAuditRule(row.id, { status: val ? 1 : 0 })
    row.status = val ? 1 : 0
